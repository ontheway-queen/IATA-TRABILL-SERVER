...
const abstract_services_1 = __importDefault(
  require('../../../../../abstracts/abstract.services')
);
  ...
  constructor() {
    ...
    this.add = (req) =>
      __awaiter(this, void 0, void 0, function* () {
        ...
        const { client_id, combined_id } = (0,
        common_helper_1.separateCombClientToId)(comb_client);
        const vouchar_no = (0, invoice_helpers_1.generateVoucherNumber)(
          7,
          'P-REF'
        );
        return yield this.models.db.transaction((trx) =>
          __awaiter(this, void 0, void 0, function* () {
            ...
            const invoice_category_id = yield conn.checkInvoiceIsPaid(
              invoice_id
            );
            const { invoice_payment } = yield conn.getInvoiceDuePayment(
              invoice_id
            );
            let airtickerPnr = [];
            let airticketRoute = [];
            ...
            for (const vendor_info of vendor_refund_info) {
              ...
              const { combined_id, vendor_id } = (0,
              common_helper_1.separateCombClientToId)(comb_vendor);
              if (vprfnd_payment_type === 'ADJUST') {
                ...
                if (vprfnd_payment_method !== 4) {
                  const total_vendor_pay =
                    yield conn.getInvoiceVendorPaymentByVendor(
                      invoice_id,
                      vendor_id,
                      combined_id
                    );
              ...
              yield conn.updateAirticketItemIsRefund(
                vprfnd_airticket_id,
                invoice_category_id,
                1
              );
