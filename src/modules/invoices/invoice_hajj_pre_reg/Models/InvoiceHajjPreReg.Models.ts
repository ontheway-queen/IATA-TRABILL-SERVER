...
class InvoiceHajjPreModels extends AbstractModels {
  public insertHajiInfo = async (insertedData: IHajiInformationDb) => {
    const id = await this.query()
      .into('trabill_haji_informations')
      .insert({ ...insertedData, trabill_org_agency: this.org_agency });

    return id[0];
  };

  updateHajiInfo = async (data: IHajiInformationDb, hajiinfo_id: idType) => {
    await this.db('trabill_haji_informations')
      .update(data)
      .where('hajiinfo_id', hajiinfo_id);
  };

  public insertHajiPreReg = async (insertedData: IInvoiceHajiPreReg) => {
    const id = await this.query()
      .into('trabill_invoice_hajj_pre_reg_haji_infos')
      .insert({ ...insertedData, haji_info_org_agency: this.org_agency });

    return id[0];
  };
  public updateHajiPreReg = async (
    insertedData: IInvoiceHajiPreReg,
    haji_info_id: idType
  ) => {
    return await this.db('trabill_invoice_hajj_pre_reg_haji_infos')
      .update(insertedData)
      .where('haji_info_haji_id', haji_info_id);
  };

  public insertHajiBillingInfo = async (insertedData: IOtherBillingInfoDb) => {
    await this.query()
      .into('trabill_invoice_hajj_pre_reg_billing_infos')
      .insert(insertedData);
  };

  public updateHajiBillingInfo = async (
    insertedData: IOtherBillingInfoDb,
    billing_id: idType
  ) => {
    await this.query()
      .update(insertedData)
  ...
  getPreRegBillingInfo = async (billing_id: idType) => {
    const data = await this.db('trabill_invoice_hajj_pre_reg_billing_infos')
      .select(
  ...
  // @GET
  public getPreRegistrationReports = async (year: string) => {
  ...
  public getHajiGroupName = async (id: number) => {
    const data = await this.query()
  ...
  public getPreviousHajiInfo = async (invoice_id: number) => {
    const data = await this.query()
      .from('trabill_invoice_hajj_pre_reg_haji_infos')
      .select(
        'haji_info_id as haji_pre_id',
        'haji_info_haji_id as haji_info_id'
      )
      .where('haji_info_invoice_id', invoice_id)
      .andWhereNot('haji_info_info_is_deleted', 1);
    return data as { haji_pre_id: number; haji_info_id: number }[];
  };

  // ====================== INVOICE HAJI PRE REGISTRATION
  public getPreHajiInfo = async (invoice_id: idType) => {
    const data = await this.query()
      .from('trabill_invoice_hajj_pre_reg_haji_infos')
      .select(
        'hajiinfo_id',
  ...
  getInvoiceHajjPreInfo = async (invoiceId: idType) => {
    const data = await this.query()
      .select(
        'invoice_client_id',
        'invoice_net_total',
  ...
  public getHajiInfoByTrackingNo = async (tracking_no: string) => {
    const data = await this.query()
      .from('trabill_invoice_hajj_pre_reg_haji_infos')
      .select(
        'hajiinfo_name',
        'hajiinfo_mobile',
        'haji_info_haji_id as haji_id',
        'haji_info_maharam',
        'hajiinfo_dob',
        'hajiinfo_tracking_number',
        'haji_info_vouchar_no',
  ...
  public getHajiIdByTrackingNo = async (tracking_no: string) => {
    const data = await this.query()
      .from('trabill_invoice_hajj_pre_reg_haji_infos')
  ...
  public getHajiInformationForHajjiManagement = async () => {
    const data = await this.query()
      .from('trabill_invoice_hajj_pre_reg_haji_infos')
      .select(
        'hajiinfo_name',
  ...
  public getHajiBillingInfos = async (invoice_id: idType) => {
  ...
  public getHajiBillingInfo = async (invoice_id: idType) => {
    const data = await this.query()
  ...
  public getForEditBillingInfo = async (invoice_id: idType) => {
    const data = await this.query()
      .into('trabill_invoice_hajj_pre_reg_billing_infos')
      .select(
        'billing_id',
  ...
  public getAllTrackingAndSerialNo = async (
    checkingType: 'tracking' | 'serial',
    value: string
  ) => {
    const haji_info = await this.query()
      .select(
        'hajiinfo_tracking_number as tracking_no',
        'hajiinfo_serial as serial_no'
      )
      .from('trabill_haji_informations')
      .where('trabill_org_agency', this.org_agency)
      .andWhereNot('hajiinfo_is_deleted', 1);

    const ummrah_haji_info = await this.query()
    ...
    if (checkingType === 'tracking') {
      const tracking_no = [...haji_info, ...ummrah_haji_info].map(
        (item) => item.tracking_no
      );
  ...
  public deletePrevHajiInfo = async (
    haji_info_id: number,
    hajiinfo_deleted_by: idType
  ) => {
    const id = await this.query()
      .into('trabill_haji_informations')
  ...
  public deleteHajiBillingInfo = async (
    invoice_id: number,
    billing_info_deleted_by: idType
  ) => {
    const id = await this.query()
      .into('trabill_invoice_hajj_pre_reg_billing_infos')
  ...
  public deleteInvoiceHajiPreReg = async (
    invoice_id: number,
    haji_info_info_deleted_by: idType
  ) => {
    const id = await this.query()
      .into('trabill_invoice_hajj_pre_reg_haji_infos')
  ...
  public restoreInvoiceHajiPreReg = async (invoice_id: number) => {
    return await this.query()
      .update({
  ...
  public deleteHajiPreRegInfo = async (
    haji_id: number,
    haji_info_info_deleted_by: idType
  ) => {
    const id = await this.query()
  ...
  public deleteSingleHajiPreReg = async (
    billing_id: idType,
    billing_info_deleted_by: idType
  ) => {
    return await this.query()
  ...
  public async updateHajjiInfoStatus(
    invoice_id: idType,
    haji_info_status: 'approved' | 'cancled',
    updated_by: idType
  ) {
    const data = await this.query()
      .update({
    ...
    if (!data) {
      throw new CustomError(
        'Please provide valid hajji invoice id',
        400,
        'Bad request'
      );
    }
  ...
  public async getAllHajiPreRegInfos(
    page: number,
    size: number,
    month: string
  ) {
    ...
    const data = await this.query()
      .select(
        'invoice_id',
        'invoice_no',
        'invoice_net_total as net_total',
        'invoice_total_vendor_price',
        'invoice_total_profit',
        this.db.raw(
          `sum(coalesce(CAST(invclientpayment_amount AS DECIMAL(10, 2)), 0)) as invclientpayment_amount`
        ),
        this.db.raw(`GROUP_CONCAT(receipt_vouchar_no) as money_receipt_num`),
        this.db.raw(`coalesce(client_name, combine_name)   AS client_name`),
        this.db.raw(`coalesce(client_mobile, combine_mobile) AS mobile`),
        'invoice_reissue_client_type',
        'invoice_sales_date as invoice_date',
        'invoice_category_id',
        'client_id',
        'invoice_is_refund',
        'haji_info_id',
      ...
      .leftJoin(
        this.db.raw(
          `trabill_money_receipts ON trabill_money_receipts.receipt_id = invclientpayment_moneyreceipt_id AND receipt_has_deleted = 0`
        )
      )
      ...
      .where(function () {
        this.where(
          'trabill_invoice_hajj_pre_reg_haji_infos.haji_info_status',
          '<>',
          'approved'
        ).orWhereNull(
          'trabill_invoice_hajj_pre_reg_haji_infos.haji_info_status'
  ...
  public async countAllHajiPreRegInfosDataRow(month: string) {
    if (month) {
      month = moment(new Date(month)).format('YYYY-MM');
    }
  ...
  public async cancelHajjPreRegHajiInfo(
    tracking_no: idType,
    invoice_id: idType
  ) {
    return await this.query()
...
export default InvoiceHajjPreModels;
