...
class ReportModel extends AbstractModels {
  ...
  // CLIENT DUE ADVANCE
  getClientWiseDueSummary = async (
    search: string,
    client_id: null | number,
    combine_id: null | number,
    page: number = 1,
    size: number = 50
  ) => {
    const offset = (+page - 1) * +size;
  ...
  getAirlineWiseClientDueSummary = async (
    search: string,
    airline_id: number,
    page: number = 1,
    size: number = 50
    ...
    const results = await this.query()
      ...
      .modify(function (queryBuilder) {
        if (search) {
          queryBuilder.where(function () {
            this.whereILike('airline_name', `%${search}%`).orWhereILike(
              'airline_code',
              `%${search}%`
            );
          });
  ...
  public async getAirTicketTotalSummary(from_date: string, to_date: string) {
    const [data] = await this.query()
